  ...
  void onInit() {
    LocalStorageServices().getUser().then((value) => {
      ...
      callRepo.getList().then((value) => {
        itens = value.obs,
        imgList.addAll(value.map((element) => buildCard(element)).toList()),
      }),
      ...
      content: Padding(
        ...
        child: Column(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.all(6.0),
              child: Text(call.title!, style: Get.textTheme.headlineSmall, overflow: TextOverflow.ellipsis, maxLines: 1,)
            ),
            Text('Setor: ${call.sector!.acronym} - ${call.sector!.name}', style: Get.textTheme.bodyLarge, overflow: TextOverflow.ellipsis, maxLines: 1,),
            Text('Prioridade: ${call.priority!.name}', style: Get.textTheme.bodyLarge, overflow: TextOverflow.ellipsis, maxLines: 1,),
            Text('Descrição: ${call.description!}', style: Get.textTheme.bodyLarge, overflow: TextOverflow.ellipsis, maxLines: 7,),
          ],
        ),
      ), 
