...
import 'package:chamados/app/features/pages/user/components/user_details_dialog_controller.dart';
...
void userDetailsDialog (UserInfoModel user) {
  Get.lazyPut(() => UserDetailsDialogController(user),);
  showDialog(
    context: Get.context!,
    builder: (_) => AlertDialog(
      titlePadding: const EdgeInsets.all(8),
      title: AppBar(title: Text(user.email!), forceMaterialTransparency: true,),
...
class EditUserPageBody extends GetView<UserDetailsDialogController> {
  ...
  Widget build(BuildContext context) {
    return Obx(() => controller.isLoading.value ? buildLoadingIndicator() : SingleChildScrollView(
          ...
          children: [
            ...
            Visibility(
              ...
              child: CheckBoxField(
                ...
                isActive: controller.user.active!
              )
            ),
            addVerticalSpace(10),
            ExpansionTile(
                    ...
                    children: [
                      ...
                      PasswordField(
                        ...
                        validator: [
                          ...
                          Validators.compare(controller.passwordEC, 'As Senhas não conferem'),
                        ]
                      ),
                    ],
                  )
            ...
            addVerticalSpace(15),
            FilledButton( child: const Text( 'Alterar Usuário',),
...
Widget dropDownButton(List<SectorModel> items, List<SectorModel> selectedItems) {
  ...
  return Obx(() =>  DropdownButtonFormField2<SectorModel>(
    decoration: const InputDecoration(labelText: 'Setores',),
    isExpanded: true,
    ...
    selectedItemBuilder: (context) {
      return items.map(
        (item) {
          return Text(selectedItems.map((element) => element.name).join(', '), maxLines: 1,);
        },
      ).toList();
    },
    dropdownSearchData: DropdownSearchData(
      ...
      searchMatchFn: (item, searchValue) => item.value.toString().contains(searchValue),
    ),
