...
import 'package:chamados/app/features/pages/user/components/user_details_dialog_provider.dart';
...
class UserDetailsDialogController extends GetxController with StateMixin<List<dynamic>> {
  ...
  @override
  void onInit() {
    LocalStorageServices().getUser().then(
      (value) {
        if (value != null) {
          logedUser = value;
          ...
          snackSucessRegister('Algo de errado aconteceu', 'se persistir entre em contato com o apoio');
        }
      }
    );
  ...
  void alterarUsuario() {
    ...
    if (dataValid) {
      UserModel userChanged = UserModel(firstname: firstNameEC.text, lastname: lastNameEC.text, phone: user.phone!, email: emailEC.text, password: passwordEC.text, sectors: selectedItens );
      _userProvider.putUser(user.id!, userChanged.toJson()).then(
        (value) => snackSucessRegister('Conta atualizada!', 'Atualização dos dados solicitados realizada'),
      ).catchError((error) {
        Get.back();
        tratarErro(error);
      }); 
