...
class CreateUpdateCategoryController extends GetxController {
  ...
  CreateUpdateCategoryController({
    this.category,
  });
  ...
  void criarAtualizar() {
    ...
    if (formValid) {
      _parametro
          ? saveItem(CallCategoryRegisterModel(
              title: tituloC.text,
              description: descricaoC.text,
              sectorId: category!.sector!.id!,
              priorityId: category!.priority!.id!))
          : saveItem(CallCategoryRegisterModel(
              title: tituloC.text,
              description: descricaoC.text,
              sectorId: category!.sector!.id!,
    ...
    if (formValid) {
      ...
      categoryRep.registerByRegisterModel(category).then((_) {
        ...
        snackSucessRegister('Registrado com sucesso',
            'Categoria ${tituloC.text} registrado com sucesso!');
      }).catchError((error) {
        tratarErro(error);
      });
    }
